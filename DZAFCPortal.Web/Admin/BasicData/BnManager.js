function BnManager() {
    ...
    //局部变量start
    var ajaxUrl = "Ajax/BasicDataHandler.ashx";
    ...
    var setting = {
        ...
        callback: {
            onClick: function (event, treeId, treeNode) {
                bnSelf.currentModel.LoadItem(treeNode);
                bnSelf.currentModel.ParentNavID =
                    typeof (treeNode.ParentID) == "undefined" ?
                    treeNode.ID : treeNode.ParentID;
                bnSelf.currentModel.CurrentNavID = treeNode.ID;
                bnSelf.currentModel.IconPath(treeNode.IconUrl);
                bnSelf.currentModel.isEdit(true);
                bnSelf.currentModel.isOnclick(true);
                var scope = treeNode.ApplyRoles == "" || treeNode == null ? "1" : "0";
                bnSelf.currentModel.visibleScope(scope);

                //上级节点名称
                var parentNode = treeNode.getParentNode();
                var val = parentNode == null ? "无" : parentNode.Name;
                bnSelf.currentModel.ParentNavName(val);
            },
            ...
            onRemove: function (event, treeId, treeNode) {
                bnSelf.currentModel.isOnclick(false);
            }
        }
    };

    bnSelf.LoadTree = function (treeID) {
    ...
    //KnockOut 绑定系列
    bnSelf.currentModel = new AppViewModel();
    var emptyGuid = "00000000-0000-0000-0000-000000000000";

    //绑定模型
    //params: roles，所有的角色
    bnSelf.BindModel = function (roles) {
        bnSelf.currentModel.Roles = JSON.parse(roles);

        ko.applyBindings(bnSelf.currentModel);
    }
